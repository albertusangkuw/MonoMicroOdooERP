...
import logging
from .calendar_event_type_abstract import MeetingTypeAbstract

_logger = logging.getLogger(__name__)

class MeetingTypeAdapter(models.Model,MeetingTypeAbstract):
    ...
    def _read(self ,field_names):
        ...
        return super(MeetingTypeAdapter,self)._read(field_names)
        ...
        for name in fields:
            ...
            if not field:
                ...
                return self.call_rpc("read", dict(http.request.httprequest.headers), self.env.context,
                                    [fields])
        return super(MeetingTypeAdapter,self).read(fields, load)
